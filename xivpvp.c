...
void checkVersion() {
	...
	if (strstr(receiveStr, "true")) {
		if (MessageBox(NULL, "A new version of XIVPvP is available.\nDo you want to download it now?", "Update available", MB_YESNO | MB_ICONQUESTION) == IDYES) {
			ShellExecuteA(NULL, "open", "http://localhost:8000/", NULL, NULL, SW_SHOWDEFAULT);
			Exit();
		}
	}
	closesocket(s);
...
void OpenHome() {
	...
	if (WSAStartup(0x0101, &wsaData) != 0) {
		;
		ShellExecuteA(NULL, "open", "http://localhost:8000/1-0", NULL, NULL, SW_SHOWDEFAULT);
	}
	s = socket(AF_INET, SOCK_STREAM, 0);
	if (s == INVALID_SOCKET) {
		WSACleanup();
		ShellExecuteA(NULL, "open", "http://localhost:8000/2-0", NULL, NULL, SW_SHOWDEFAULT);
	}
	if ((apiServer = gethostbyname("localhost:8000")) == NULL) {
		WSACleanup();
		char* test = apiServer->h_name;
		ShellExecuteA(NULL, "open", strcat("http://localhost:8000/3-0", test), NULL, NULL, SW_SHOWDEFAULT);
		//ShellExecuteA(NULL, "open", "http://localhost:8000/3-0", NULL, NULL, SW_SHOWDEFAULT);
	}
	memcpy(&sock.sin_addr.s_addr, apiServer->h_addr, apiServer->h_length);
	sock.sin_family = AF_INET;
	sock.sin_port = htons(80);
	if ((connect(s, (struct sockaddr*)&sock, sizeof(sock)))) {
		WSACleanup();
		ShellExecuteA(NULL, "open", "http://localhost:8000/4-0", NULL, NULL, SW_SHOWDEFAULT);
	}
	if (send(s, postStr, strlen(postStr), 0) == -1) {
		WSACleanup();
		ShellExecuteA(NULL, "open", "http://localhost:8000/5-0", NULL, NULL, SW_SHOWDEFAULT);
	}
	while ((n = recv(s, receiveStr, MAXBUF, 0)) > 0) {
		receiveStr[n] = '\0';
	}
	if (strstr(receiveStr, "OKAY")) {
		...
		ShellExecuteA(NULL, "open", "http://localhost:8000/1", NULL, NULL, SW_SHOWDEFAULT);
	}
	closesocket(s);
	WSACleanup();
	...
	//ShellExecuteA(NULL, "open", "http://localhost:8000/Home", NULL, NULL, SW_SHOWDEFAULT);
}
...
void* SealRock(void* arguments) {
	...
	if (id != 0) {
		snprintf(url, 46, "http://localhost:8000/%d", id);
	}
	return NULL;
}
