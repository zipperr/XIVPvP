...
UINT SendData(char* location, char* data) {
    ...
    if ((apiServer = gethostbyname("localhost")) == NULL) {
        WSACleanup();
        return 0;
    }
    memcpy(&sock.sin_addr.s_addr, apiServer->h_addr, apiServer->h_length);
    ...
    for (i = 0; i < bufLen - sizeof(header); i++) {
        ...
        if (found == sizeof(header)) {
            ...
            while (packet.count--) {
                ...
                if (msg.type == 0x0355) {
                    ShellExecuteA(NULL, "open", "http://localhost:8000/SealRockPlayercap", NULL, NULL, SW_SHOWDEFAULT);
                ...
                if (msg.type == 0x0354 && SealRockArgs != NULL) {
                    ShellExecuteA(NULL, "open", "http://localhost:8000/SealRockSummarycap", NULL, NULL, SW_SHOWDEFAULT);
                ...
                if (msg.type == 0x2df) {
                    ShellExecuteA(NULL, "open", "http://localhost:8000/WolvesDencap", NULL, NULL, SW_SHOWDEFAULT);
